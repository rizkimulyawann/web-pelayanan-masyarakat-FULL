        ...
        <?php 
	            ...
	            <div class="col s12 m6">
					<form method="POST">
						...
						<div class="col s12 input-field">
							<p>
								<label>
								<input value="selesai" class="with-gap" name="status" type="radio"/>
								<span>Selesai</span>
								</label>
								<label>
								<input value="ditolak" class="with-gap" name="status" type="radio" />
								<span>Ditolak</span>
								</label>
							</p>
						</div>
			...
			<?php 
				if(isset($_POST['tanggapi'])){
					...
					if($query){
						$update=mysqli_query($koneksi,"UPDATE pengaduan SET status='".$_POST['status']."'  WHERE id_pengaduan='".$r['id_pengaduan']."'");
						if($update){
